// c++ -o Reading Reading.cpp `root-config --libs --glibs --cflags`
#include "LHEF.h"
#include<iostream>
#include "TH1.h"
#include "TFile.h"
...
int main(int argc, char** argv){
	...
	tree->Branch("deltaeta_bb", &deltaeta_bb);
	tree->Branch("deltaeta_ljj", &deltaeta_ljj);
	tree->Branch("deltaeta_bbljj", &deltaeta_bbljj);
			...
			if(jets.size() > 0 && jets.size() < 3 && tau == 0){      
            	for (int i=0; i<jets.size(); i += 2){
                       ...
                       //    cout << "j2 :  " << j2_eta << endl;
						...
						deltaeta_bbljj = (b1_eta + b2_eta) - (lep_eta + j1_eta + j2_eta);
...
//			cout << "Event " << iEv << " is not saved. " << endl;
			...
			for (int iPart = 0 ; iPart < reader.hepeup.IDUP.size (); iPart++){
                ...
                if (reader.hepeup.ISTUP.at (iPart) == 1){
                    ...
                    if(ID == 11){
                        if(lep_pt > 15 && lep_eta < 3 && abs(j1_eta) < 4.5 && abs(j2_eta) < 4.5 && j1_pt > 30 && j2_pt> 30 && mjj > 60){
                     ...
                     if(ID == 13){
                        if(lep_pt > 15 && lep_eta < 2.4 && abs(j1_eta) < 4.5 && abs(j2_eta) < 4.5 && j1_pt > 30 && j2_pt> 30 && mjj > 60){
